            ...
            <div class="form-group">
              ...
              <div class="date-picker-wrapper" ref="datePickerRef">
                <input
                  ...
                  @input="form.date = $event.target.value; showDatePicker = false"
                  type="text"
                  placeholder="选择日期"
                  readonly
                ...
                <div v-if="showDatePicker" class="date-picker-popup">
                  <div class="dp-header">
                    <button @click="dpPrevYear" class="dp-btn dp-btn-sm" type="button">‹‹</button>
                    <button @click="dpPrevMonth" class="dp-btn" type="button">‹</button>
                    <span class="dp-title">{{ dpYear }}年 {{ dpMonth }}月</span>
                    <button @click="dpNextMonth" class="dp-btn" type="button">›</button>
                    <button @click="dpNextYear" class="dp-btn dp-btn-sm" type="button">››</button>
                  </div>
                  ...
                  <div class="dp-days">
                    <span
                      v-for="(d, i) in dpDays" :key="i"
                      :class="[
                        'dp-day',
                        { 'dp-day-other': d.other },
                        { 'dp-day-today': d.today },
                        { 'dp-day-selected': d.selected },
                        { 'dp-day-weekend': d.weekend }
                      ]"
                      @click="d.other || selectDate(d)"
                    >{{ d.day }}</span>
                  </div>
                  <div class="dp-footer">
                    <button @click="selectToday" class="dp-btn-today" type="button">今天</button>
                    <button @click="showDatePicker = false; form.date = ''" class="dp-btn-clear" type="button">清除</button>
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  setup() {
    ...
    const dpDays = computed(() => {
      const y = dpYear.value
      const m = dpMonth.value
      const firstDay = new Date(y, m - 1, 1).getDay() // 0=Sun
      const daysInMonth = new Date(y, m, 0).getDate()
      const daysInPrevMonth = new Date(y, m - 1, 0).getDate()
      const today = todayStr()
      const sel = form.value.date
      // 周一开始，offset: Mon=0, Sun=6
      const startOffset = firstDay === 0 ? 6 : firstDay - 1
      const result = []
      // 上月日期
      for (let i = startOffset - 1; i >= 0; i--) {
        const d = daysInPrevMonth - i
        result.push({ day: d, other: true, val: '' })
      }
      // 本月日期
      for (let d = 1; d <= daysInMonth; d++) {
        const pad = String(d).padStart(2, '0')
        const padM = String(m).padStart(2, '0')
        const val = `${y}-${padM}-${pad}`
      ...
      while (result.length < totalCells) {
        result.push({ day: next++, other: true, val: '' })
      }
      return result
    })

    const selectDate = (d) => {
      form.value.date = d.val
    ...
    const selectToday = () => {
      ...
      const t = new Date()
      dpYear.value = t.getFullYear()
      dpMonth.value = t.getMonth() + 1
      showDatePicker.value = false
    }
    ...
    const dpPrevYear = () => dpYear.value--
    const dpNextYear = () => dpYear.value++
    ...
    return {
      ...
      exportBackup,
      importBackup,
      showDatePicker, dpYear, dpMonth, dpDays, datePickerRef,
      selectDate, selectToday, dpPrevMonth, dpNextMonth, dpPrevYear, dpNextYear,
      // 密码修改
      currentPassword,
      newPassword,
      confirmPassword,
...
.dp-day-weekend:not(.dp-day-selected):not(.dp-day-other) {
  color: #ef4444;
}
